...
export class Application extends Model<Application> {
  ...
  /**
   * Get the average loan amount from all applications
   */
  static async getAverageLoanAmount(): Promise<number> {
    ...
    // Retrieve the average loan amount from the result
    const averageLoanAmount = result?.get('averageLoanAmount') || 0;

    // Convert to a number and return
    return parseFloat(averageLoanAmount as string);
  }
}
